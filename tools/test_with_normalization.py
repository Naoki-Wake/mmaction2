...
def inference_pytorch(args, cfg, distributed, data_loader):
    ...
    for i in range(len(bias)):
        model.cls_head.fc_cls.bias[i] = 0.0
    # normalize each row by L2 norm
    for row in range(len(bias)):
        weight[row,:] = weight[row,:]/np.linalg.norm(weight[row,:], ord=2)
        weight[row,:] = weight[row,:]/np.sum(weight[row,:])
    for i in range(weight.shape[0]):
        for j in range(weight.shape[1]):
            model.cls_head.fc_cls.weight[i][j] = float(weight[i,j])
